...
3. **Natural-language Quick Add**: the §25 parser always runs first and wins for every field it recognised. With the
   switch on, the model may fill only fields the parser left empty (amount, type, category, date within ±31 days,
   description); every value is validated and shown in the editable fields before Save, never saved directly.
4. **Analytics narrative**: a collapsed "Summary" section with a Generate button, built only from the report's
   already-computed numbers (the model never sees individual transactions). Hidden when the switch is off.
...
(AI never writes; validation) · ☐ PROGRESS + PR · ☐ WALK-QUEUE (on-device AI checks on a real device)
