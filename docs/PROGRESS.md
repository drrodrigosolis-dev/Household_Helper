...
| 10 — Hardening | CI green (device checks queued) | Runs 36223806070/36223803375 on `c9230f9`: lint, build, 210 unit and 40 UI tests green. Reviews done: privacy, accessibility, destructive actions, data safety (twice), compile, security, v1 completeness audit. Fixed: Face ID gate + app-switcher cover (the Log Transaction shortcut honours it; the lock never switches itself off); non-drag reordering; VoiceOver labels; dark-mode chart contrast; rollback guard on every service write; atomic category move-and-delete; link cleanup; restore keeping intact photos; privacy manifests; backups validated before any photo is read, photos past a 500 MB cap count as missing, restored photos re-encoded, photos stored with complete file protection; Sprint 9 v1 gaps (task↔transaction link, appearance, default Quick Add type, About); v1 audit gaps (wishlist↔task links both ways, linked tasks in the transaction editor, Settings › Household for the starting balance/date and currency, Quick Add on the Current balance card, Recent activity rows open their tab, recurring series edit and delete) with UI tests for every flow the audit found untested. Reduce Motion: no custom animations (checked in code). Remaining: device-only checks and the launch-time NFR (WALK-QUEUE, MOVING-TO-MAC). |
